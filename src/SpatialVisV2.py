...
def download_svg(url, output_path):
    """
    A funtion that downloads a SVG file from a URL to a specified path.
        =====
        Inputs:
            url: An internet link to a SVG image.
            output_path: The location where the SVGs are saved.
    """
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to download {url} to {output_path}")
...
def prepare_analysis(excel_file, image_folder, sID, background_folder):
    """Updated function using Inkscape for conversion, now skips steps if output exists."""
    import os
    # 1. Download student sketches (SVGs)
    sketches_dir = os.path.join(image_folder, f'{sID}_rsketches')
    skip_download = os.path.exists(sketches_dir) and any(os.scandir(sketches_dir))
    if skip_download:
        print(f"Skipping download: {sketches_dir} already exists and is not empty.")
    else:
        batch_download(excel_file, image_folder, sheet_index=f'{sID}')

    # 2. Overlay sketches on backgrounds (SVG)
    svg_output = os.path.join(image_folder, f'{sID}_svg')
    skip_overlay = os.path.exists(svg_output) and any(os.scandir(svg_output))
    if skip_overlay:
        print(f"Skipping overlay: {svg_output} already exists and is not empty.")
    else:
    ...
    skip_convert = os.path.exists(png_output) and any(os.scandir(png_output))
    if skip_convert:
        print(f"Skipping PNG conversion: {png_output} already exists and is not empty.")
    else:
        convert_svgs_to_pngs_inkscape(
            input_folder=svg_output,
            output_folder=png_output,
            student=True
        )

    # 4. Clean Excel data (always run)
...
#    clean_data(excel_file, f'{sID}')
def run_analysis(image_folder, excel_file, start_index, sID, load_in=False, if_exists='replace'):
    """
    Initialize and run the grading GUI for a given student.
    """
    import ast
    import pandas as pd
    import os
    ...
    for i, row in df.iterrows():
        msg, corr, miss, add = get_results(row['grading_metrics'])
        ...
        # default, unevaluated analysis record
    ...
    # 2) Optionally load previous "<sID>_output" sheet and overwrite defaults
    if load_in:
        try:
            af = pd.read_excel(excel_file, sheet_name=f'{sID}_output')
            loaded = []
            for j, row in af.iterrows():
                const = ast.literal_eval(row['constructor_dict'])
                loaded.append({
                    'assignment_id':    row['id'],
                    'index':            j,
                    ...
                    # copy each multi‐select “change” field
                    'first_attempt':   const.get('first_attempt',''),
                    'no_change':       const.get('no_change',''),
                    'clean_up_lines':  const.get('clean_up_lines',''),
                    'add_line':        const.get('add_line',''),
                    'remove_line':     const.get('remove_line',''),
                    're_orienting':    const.get('re_orienting',''),
                    're_sizing':       const.get('re_sizing',''),
                    'other1':          const.get('other1',''),

                    # copy each multi‐select “mistakes” field
                    'no_mistakes':     const.get('no_mistakes',''),
                    'extra_line':      const.get('extra_line',''),
                    'extra_section':   const.get('extra_section',''),
                    'extra_dashes':    const.get('extra_dashes',''),
                    'missing_line':    const.get('missing_line',''),
                    'missing_section': const.get('missing_section',''),
                    'missing_dashes':  const.get('missing_dashes',''),
                    'wrong_size':      const.get('wrong_size',''),
                    'wrong_orientation':const.get('wrong_orientation',''),
                    'wrong_location':  const.get('wrong_location',''),
                    'messy_lines':     const.get('messy_lines',''),
                    'f_not_r':         const.get('f_not_r',''),
                    'other2':          const.get('other2',''),

                    # copy each comment field
            ...
            if loaded:
                analysis_list = loaded
                print(f"Loaded {len(loaded)} saved analyses from '{sID}_output'")
            else:
                print(f"No saved analyses in '{sID}_output'; using fresh defaults.")
        except ValueError:
            print(f"No previous output sheet '{sID}_output'; using fresh defaults.")
    ...
    class SpatialVisViewer(tk.Tk):
        ...
        def __init__(self, data, analysis_list, start_index):
            ...
            self.attempt_sel_var = tk.StringVar()
            self.attempt_selector_label = ttk.Label(self.control_frame,
                                                    text="Select attempt:")
            # Sort self.data by assignment_code, then attempt
            self.data.sort(key=lambda d: (d['assignment_code'], d['attempt']))

            self.attempt_selector = ttk.Combobox(
                self.control_frame,
                textvariable = self.attempt_sel_var,
                state       = "readonly",
                values      = [f"{d['assignment_code']} - Attempt {d['attempt']+1}" for d in self.data]
            )
            self.attempt_selector.current(self.current_index)
            self.attempt_selector.bind("<<ComboboxSelected>>",
                                       self.jump_to_attempt)
            # place the selector in row-0 of the control frame
            self.attempt_selector_label.grid(row=0, column=0, sticky='e', padx=5)
            self.attempt_selector.grid       (row=0, column=1, sticky='we', padx=5)
        ...
        def update_content(self):
            ...
            last_additional = f'Preious additional percent: {(last_item["additional_per"] * 100):.0f}'
            ...
            self.correctness_combo.set(analysis_item['correctness'])
            self.grading_combo.set(analysis_item['grading'])
            self.hints_combo.set(analysis_item['mini_hints'])
            self.learning_combo.set(analysis_item['evidence'])
            self.act_check1_var.set(analysis_item['first_attempt'])
            self.act_check2_var.set(analysis_item['no_change'])
            self.act_check3_var.set(analysis_item['clean_up_lines'])
            self.act_check4_var.set(analysis_item['add_line'])
            self.act_check5_var.set(analysis_item['remove_line'])
            self.act_check6_var.set(analysis_item['re_orienting'])
            self.act_check7_var.set(analysis_item['re_sizing'])
            self.act_check8_var.set(analysis_item['other1'])
            self.mis_check1_var.set(analysis_item['no_mistakes'])
            self.mis_check2_var.set(analysis_item['extra_line'])
            self.mis_check3_var.set(analysis_item['extra_section'])
            self.mis_check4_var.set(analysis_item['extra_dashes'])
            self.mis_check5_var.set(analysis_item['missing_line'])
            self.mis_check6_var.set(analysis_item['missing_section'])
            self.mis_check7_var.set(analysis_item['missing_dashes'])
            self.mis_check8_var.set(analysis_item['wrong_size'])
            self.mis_check9_var.set(analysis_item['wrong_orientation'])
            self.mis_check10_var.set(analysis_item['wrong_location'])
            self.mis_check11_var.set(analysis_item['messy_lines'])
            self.mis_check13_var.set(analysis_item['f_not_r'])
            self.mis_check12_var.set(analysis_item['other2'])
            self.hint_comment_text.delete('1.0', tk.END)
            self.hint_comment_text.insert('1.0', analysis_item['hint_comment'])
            self.evidence_comment_text.delete('1.0', tk.END)
            self.evidence_comment_text.insert('1.0', analysis_item['evidence_comment'])
            self.change_comment_text.delete('1.0', tk.END)
            self.change_comment_text.insert('1.0', analysis_item['action_comment'])
            self.mistake_comment_text.delete('1.0', tk.END)
            self.mistake_comment_text.insert('1.0', analysis_item['mistakes_comment'])
            self.gen_comment_text.delete('1.0', tk.END)
            self.gen_comment_text.insert('1.0', analysis_item['gen_comment'])
        ...
        def next_image(self):
            ...
            self.update_content()
            # sync selector
            if hasattr(self, 'attempt_selector'):
                self.attempt_selector.current(self.current_index)
        ...
        def jump_to_attempt(self, event=None):
            """Jump directly to the attempt chosen in the drop-down."""
            self.update_analysis()                      # save any current edits
            self.current_index = self.attempt_selector.current()
            self.update_content()
